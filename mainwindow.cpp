...
void MainWindow::tabPageChanged(int index)
{
    ...
    if (index > 0)
    {
        //qDebug() << "tbbbb"<< m_builder->getGraphModel()->getfolderename() << m_builder->getGraphModel()->getfilename();
        ...
        if (m_workingGraphModel == NULL)
        {
            ...
            else
            {
                ...
                //qDebug() << "tatataaa"<< m_zzCalculatorPanel-> getfolderename() << m_zzCalculatorPanel-> getfilename();
            }
