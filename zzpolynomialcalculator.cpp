...
void ZZPolynomialCalculator::run()
{
    ...
    QString zzcalcname = "StarPolyCalc.exe";
...
void ZZPolynomialCalculator::run2()
{
    ...
    QString zzcalcname2 = "ZZPolyCalc.exe";

    m_poly2.clear();
    QFileInfo cmdInfo = QFileInfo(QFileInfo(QCoreApplication::arguments().at(0)).absolutePath(),zzcalcname2);

    if (!cmdInfo.exists() || !cmdInfo.isExecutable())
    {
        m_hasError = true;
        m_errorMsg = zzcalcname2 + " not found in the same path as FullereneDecomposer";
        return;
    }
    QString cmd = cmdInfo.absoluteFilePath() ;
...
//    arguments <<"-Q";
    ...
    if (file.open())
    {
        ...
        m_process2->start(cmd, arguments);
        m_process2->waitForFinished(-1);

        if (m_process2->exitCode()!=0)
        {
            m_hasError = true;
            m_errorMsg = zzcalcname2 + " crashed.";
            return;
        }

        QByteArray result = m_process2->readAll();
        QString t(result);
        QDomDocument resultXML;
        bool failed = !resultXML.setContent(t);
        if (failed)
        {
            m_hasError = true;
            m_errorMsg = "The output from " + zzcalcname2 + " has wrong format1.";
            return;
        }
        ...
        {
            m_hasError = true;
            m_errorMsg = "The output from " + zzcalcname2 + " has wrong format2.";
            qDebug() << t;
            return;
        }
        ...
        for (int i = 0; i < terms.count(); ++i)
        {
            ...
            {
                m_hasError = true;
                m_errorMsg = "The output from " + zzcalcname2 + " has wrong format3.";
                qDebug() << t;
                return;
            }
            QDomElement orderElem = orders.at(0).toElement();
